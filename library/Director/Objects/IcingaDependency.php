    ...
    public function getOnDeleteUrl()
    {
        if ($this->child_service_id) {
            if ($this->hasBeenAssignedToServiceApply()) {
                return 'director/service/dependencies?id=' . (int) $this->child_service_id;
            } else {
                return 'director/service/dependencies?name='
                    . rawurlencode($this->child_service)
                    . '&host='
                    . rawurlencode($this->child_host);
            }
